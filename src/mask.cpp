...
bool Santa::Mask::is_colliding(int x1, int y1, int frame1,
                        Mask *mask2, int x2, int y2, int frame2) {
    ...
    //Output::debug("c: %i, test_x1: %i, test_y1: %i, test_x2: %i, test_y2: %i\n", clip_h, test_x1, test_y1, test_x2, test_y2);
...
int Santa::Mask::get_width(void) {
    return m_width;
}

int Santa::Mask::get_height(void) {
    return m_height;
}

bool **Santa::Mask::get_array(void) {
    return m_array;
}
