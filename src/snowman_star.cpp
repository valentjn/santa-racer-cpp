...
void Santa::SnowmanStar::reinit(bool first_init) {
    ...
    m_level_x = m_snowman->get_level_x() +
                snowman_rocket_offset_x -
    ...
    if (first_init) {
        m_frame = m_max_frame -
                  Random::rnd(wait_frame_count_min, wait_frame_count_max);
    } else {
        m_frame = 0;
    }
...
void Santa::SnowmanStar::draw(void) {
    ...
    Draw::blit(m_surface, frame * m_width, 0, m_width, m_height,
      Setup::screen, m_level_x - Setup::game->level->get_offset(), m_y);
}
