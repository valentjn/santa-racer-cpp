...
void Santa::Game::init(void) {
    ...
    m_highscore_bg = SDL_CreateRGBSurface(SDL_SWSURFACE, 400, 300, 32, 0, 0, 0,
                                          0);
            ...
            if (mode == new_highscore_mode) {
                if ((event.key.keysym.sym == SDLK_BACKSPACE) ||
                    ...
                    if (m_highscore_name.length() > 0) {
                        m_highscore_name = m_highscore_name.erase(
                            m_highscore_name.length() - 1);
...
void Santa::Game::logic(void) {
    ...
    if (mode == running_game) {
        ...
        if (m_won_lost_time + won_splash_duration <= SDL_GetTicks()) {
            ...
            if (mode == new_highscore_mode) {
                ...
                Config::highscores[m_highscore_place].name = "";
                Config::highscores[m_highscore_place].score =
                        m_highscore_score;
            } else {
                return_to_menu();
                mode = highscore_mode;
    ...
    if (SDL_GetTicks() >= m_bell_time) {
        ...
        m_bell_time = SDL_GetTicks() +
                      Random::rnd(bell_time_min, bell_time_max);
    }
    if (SDL_GetTicks() >= m_dog_time) {
        Sound::play_volume("dog", dog_volume);
        m_dog_time = SDL_GetTicks() +
                     Random::rnd(dog_time_min, dog_time_max);
    }
}
...
void Santa::Game::draw(void) {
    ...
    if ((mode == menu_mode) || (mode == highscore_mode)) {
        Draw::copy(Setup::images["logo"], Setup::screen,
                   0, Setup::screen_height - Setup::images["logo"]->h);
    }
...
void Santa::Game::draw_text(void) {
    ...
    if (mode == menu_mode || mode == highscore_mode) {
        ...
        Setup::text->draw("F3 - Highscores", Setup::screen_width / 2, 0,
                          Text::TopCenter);
        Setup::text->draw("F5 - Spielen", Setup::screen_width, 0,
                          Text::TopRight);
    ...
    if (countdown_mode) {
        ...
        Setup::text->draw(string, sleigh->get_x() - 10,
          sleigh->get_y() + sleigh->get_height() / 2, Text::CenterRight);
    }
}
...
void Santa::Game::draw_highscores(void) {
    ...
    line_spacing = (highscore_height -
      Setup::text->get_line_height() - 2 * highscore_padding_y) / 9;
      
    Draw::blit(m_highscore_bg, 0, 0, highscore_width,
               highscore_height, Setup::screen, x, y);
    ...
    for (i = 0; i < 10; i++) {
        ...
        Setup::text->draw(name.c_str(), x, y,
                          Text::TopLeft);
        
        Setup::text->draw(score_string, x + highscore_width -
                          2 * highscore_padding_x, y, Text::TopRight);
