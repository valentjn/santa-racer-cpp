...
void Santa::Sleigh::draw(void) {
    ...
    if (is_invincible()) {
        time = (m_invincible_end - SDL_GetTicks()) %
               invincible_invisible_period;
    ...
    Draw::blit(m_sleigh_surface, frame * m_sleigh_width, 0,
               m_sleigh_width, m_sleigh_height, Setup::screen, x, y);
    
    Draw::blit(m_reindeer_surface,
      reindeer_frame_number * m_reindeer_width, 0, m_reindeer_width,
      m_reindeer_height, Setup::screen,
      x + m_sleigh_width + reindeer_offset_x,
      y + reindeer_offset_y);
    
    Draw::blit(m_reindeer_surface, frame * m_reindeer_width, 0,
      m_reindeer_width, m_reindeer_height, Setup::screen,
      x + m_sleigh_width, y + reindeer_offset_y);
    ...
    if (has_shield()) {
        shield_frame = int((m_shield_end - SDL_GetTicks()) / 1000.0 *
                           shield_frame_speed) % shield_frame_count;
...
bool Santa::Sleigh::is_colliding(void) {
    ...
    for (tile_y = 0; tile_y < level_height; tile_y++) {
        for (tile_x = start_x; tile_x < start_x + tiles_to_draw; tile_x++) {
            ...
            if (m_sleigh_mask->is_colliding(x, y, sleigh_frame,
                level_mask, tile_x * tile_width - level_offset,
                tile_y * tile_height, level_frame)) {
                
                return true;
            }
            
            if (m_reindeer_mask->is_colliding(x + m_sleigh_width +
                reindeer_offset_x, y, reindeer_frame,
                level_mask, tile_x * tile_width - level_offset,
                tile_y * tile_height, level_frame)) {
...
Santa::LevelObject *Santa::Sleigh::touched_level_object(void) {
    ...
    for (i = 0; i < Level::max_level_object_count; i++) {
        ...
        if (object->exists() && !object->is_hidden()) {
            ...
            if (m_sleigh_mask->is_colliding(x, y, sleigh_frame,
                object_mask, object_level_x - level_offset,
                object_y, object_frame)) {
...
int Santa::Sleigh::get_x(void) {
    ...
    if (m_menu_mode) {
        return (sin(float(SDL_GetTicks() - m_time) / menu_x_period_length * 2 *
                M_PI + m_rand_offset_x) + 1) *
                ((menu_max_x - menu_min_x) / 2.0) + menu_min_x;
    } else {
        if (m_speed_x == 0) {
            return m_x;
        } else {
...
int Santa::Sleigh::get_y(void) {
    ...
    if (m_menu_mode) {
        return (sin(float(SDL_GetTicks() - m_time) / menu_y_period_length * 2 *
                M_PI + m_rand_offset_y) + 1) *
                ((menu_max_y - menu_min_y) / 2.0) + menu_min_y;
    } else {
        if (m_speed_y == 0) {
            return m_y;
...
int Santa::Sleigh::get_speed_x(void) {
    return m_speed_x;
}

int Santa::Sleigh::get_speed_y(void) {
    return m_speed_y;
}

int Santa::Sleigh::get_width(void) {
    return m_width;
}

int Santa::Sleigh::get_height(void) {
    return m_height;
}
...
void Santa::Sleigh::set_menu_mode(bool menu_mode) {
...
bool Santa::Sleigh::get_pause(void) {
    return m_pause;
}
